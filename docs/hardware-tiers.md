...
On constrained machines, `scripts/generate-mlx-text.sh` and `scripts/serve-mlx.sh` set `mx.set_memory_limit` and `mx.set_wired_limit` to `max_recommended_working_set_size` and `mx.set_cache_limit` to 256 MiB in the inference process, and select the GPU, before weights load. `make validate` probes those same APIs and does **not** exceed that Metal recommended working set (this M1: 5726633984 bytes). Image and video wrappers do **not** inherit the cap; those workloads may need swap beyond the working set. `make detect` prints the working set when mlx is importable.
